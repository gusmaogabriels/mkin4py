...
import pytest
from mkin4py import _usage


def test_disabled_means_no_writes(tmp_path, monkeypatch):
    monkeypatch.delenv('MKIN4PY_USAGE_DB', raising=False)
    monkeypatch.chdir(tmp_path)
    assert _usage.run(lambda _: 0, ['solve', 'private-model.json'], package='mkin4py', version='test', commands={'solve'}) == 0
    assert list(tmp_path.iterdir()) == []
    assert _usage.report('mkin4py')['enabled'] is False


def test_counts_exit_codes_without_inputs(tmp_path, monkeypatch, capsys):
    path = tmp_path/'counts.sqlite3'
    monkeypatch.setenv('MKIN4PY_USAGE_DB', str(path))
    for code in (0, 1, 2):
        assert _usage.run(lambda _, c=code: c, ['solve', '/sensitive/workbook.xlsx'],
            package='mkin4py', version='test', commands={'solve'}) == code
    assert _usage.run(lambda _: 0, ['sensitive-formula'], package='mkin4py', version='test', commands={'solve'}) == 0
    result = _usage.report('mkin4py')
    assert result['runs'] == 4
    assert {row['command'] for row in result['counts']} == {'solve', 'invalid'}
    assert b'sensitive' not in path.read_bytes()
    assert _usage.run(None, ['usage', '--json'], package='mkin4py', version='test', commands=set()) == 0
    assert json.loads(capsys.readouterr().out)['runs'] == 4
    assert _usage.report('mkin4py')['runs'] == 4


def test_argparse_exit_and_storage_failure_preserve_exit_code(tmp_path, monkeypatch, capsys):
    monkeypatch.setenv('MKIN4PY_USAGE_DB', str(tmp_path/'counts.sqlite3'))
    def invalid(_): raise SystemExit(2)
    with pytest.raises(SystemExit) as error:
        _usage.run(invalid, ['solve'], package='mkin4py', version='test', commands={'solve'})
    assert error.value.code == 2
    assert _usage.report('mkin4py')['counts'][0]['exit_code'] == 2
    monkeypatch.setenv('MKIN4PY_USAGE_DB', str(tmp_path/'missing'/'counts.sqlite3'))
    assert _usage.run(lambda _: 0, ['solve'], package='mkin4py', version='test', commands={'solve'}) == 0
    assert 'could not update' in capsys.readouterr().err


def test_unresolvable_path_keeps_command_result(monkeypatch, capsys):
    monkeypatch.setenv('MKIN4PY_USAGE_DB', '~no-such-usage-account/counts.sqlite3')
    assert _usage.run(lambda _: 0, ['solve'], package='mkin4py', version='test', commands={'solve'}) == 0
    assert 'could not update' in capsys.readouterr().err


def test_cli_usage_report_does_not_count_itself(tmp_path, monkeypatch, capsys):
    from mkin4py.cli import main
    monkeypatch.setenv('MKIN4PY_USAGE_DB', str(tmp_path/'counts.sqlite3'))
...
def test_database_handles_are_released_for_removal(tmp_path, monkeypatch):
    path = tmp_path/'counts.sqlite3'
    monkeypatch.setenv('MKIN4PY_USAGE_DB', str(path))
    _usage._record('mkin4py', 'test', 'solve', 0, .1)
    assert _usage.report('mkin4py')['runs'] == 1
...
def test_concurrent_cli_processes_and_readonly_reporting(tmp_path, monkeypatch):
    path = tmp_path/'counts.sqlite3'
    monkeypatch.setenv('MKIN4PY_USAGE_DB', str(path))
    assert _usage.report('mkin4py')['runs'] == 0
    assert not path.exists()
    source = "from mkin4py._usage import _record; [_record('mkin4py','test','solve',0,.01) for _ in range(20)]"
...
def test_contention_commits_once_or_rolls_back(tmp_path, monkeypatch, lock, expires):
    ...
    with closing(sqlite3.connect(path, isolation_level=None, timeout=0.)) as blocker:
        ...
        if expires:
            with pytest.raises(sqlite3.OperationalError) as error:
                _usage._record('mkin4py', 'test', 'solve', 0, .2)
    ...
    _usage._record('mkin4py', 'test', 'solve', 0, .1)
    assert _usage.report('mkin4py')['runs'] == (2 if expires else 3)
